...
function jacobian_matrices = find_jacobian_matrices(eqbm_points, x1_dot, x2_dot)
  ...
  ################## ADD YOUR CODE HERE ######################
  J = jacobian([x1_dot;x2_dot]);
  for k = 1:length(solutions)
    #subs(J , {x1, x2} , {solutions{k}(1), solutions{k}(2)})
    jacobian_matrices{k} = double((subs(J , {x1, x2} , {solutions{k}(1), solutions{k}(2)})));
...
function [eigen_values stability] = check_eigen_values(eqbm_pts, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    flag = 1;
    ################## ADD YOUR CODE HERE ######################
    eigen_values{k} = double(eig(sym(matrix)));
    ############################################################
    if (flag == 1 && real(eigen_values{k}) <= 0)
